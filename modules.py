...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        if config.mod_method == 'bpsk':
            self.layer4 = self.make_layer(ResidualBlock, config.trans_bit, 2, stride=2)
        else:
            self.layer4 = self.make_layer(ResidualBlock, config.trans_bit * 2, 2, stride=2)
...
class Decoder_Recon(nn.Module):
    def __init__(self, config):
        ...
        if config.mod_method == 'bpsk':
            input_channel = int(config.trans_bit / (4 * 4))
        else:
            input_channel = int(config.trans_bit * 2 / (4 * 4))
